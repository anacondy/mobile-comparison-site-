...
export default function AISmartphoneCompare() {
  ...
  const hfToken = process.env.REACT_APP_HF_API_TOKEN; // optional
  const [status, setStatus] = useState('');
  const [error, setError] = useState('');
  const [provider, setProvider] = useState('openrouter'); // 'openrouter' | 'hf'
  ...
  // Use CRA dev proxy (set in my-app/package.json) to hit /api/* on the same origin during dev
  const API_BASE = '';
  ...
  const r = await fetch(`/api/search?q=${encodeURIComponent(q)}`);
      ...
      const items = (j.data?.phones || []).slice(0, 6);
      which === 'A' ? setResultsA(items) : setResultsB(items);
      if (items.length === 0) setStatus(`No results for "${q}"`);
      else setStatus('');
    } catch (e) {
      console.warn(e);
      setError('Search failed. Check your connection or try a different query.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function suggest(which, q) {
    ...
    try {
      const r = await fetch(`/api/ai/suggest?q=${encodeURIComponent(q)}&provider=openrouter`);
      const j = await r.json();
      const list = (j.suggestions || []).slice(0, 6);
  ...
  async function pick(which, item) {
    ...
    try {
      // Ensure we hit the v2 detail endpoint; search returns a slug like "apple_iphone_13-1110"
  // Always go through our proxy using the slug to avoid mixed origins
  const r = await fetch(`/api/detail?slug=${encodeURIComponent(item.slug)}`);
      if (!r.ok) throw new Error(`Detail fetch failed (${r.status})`);
      const j = await r.json();
      which === 'A' ? setA(j.data) : setB(j.data);
      setStatus('');
    } catch (e) {
      console.warn(e);
  ...
  function deriveMetrics(spec, sections) {
    if (!spec) return null;
    const display = sections.display.reduce((acc, x) => ({ ...acc, [x.label]: x.value }), {});
    const hardware = sections.hardware.reduce((acc, x) => ({ ...acc, [x.label]: x.value }), {});
    const battery = sections.battery.reduce((acc, x) => ({ ...acc, [x.label]: x.value }), {});

    const sizeIn = numFrom(display.Size, /(\d+(?:\.\d+)?)\s*(?:in|inch|inches)/i);
    const hz = numFrom(display['Refresh Rate'], /(\d{2,3})\s*Hz/i);
    const ram = numFrom(hardware.RAM, /(\d+(?:\.\d+)?)\s*GB/i);
    const storage = numFrom(hardware.Storage, /(\d+(?:\.\d+)?)\s*GB/i);
    const mAh = numFrom(battery.Capacity, /(\d{3,5})\s*mAh/i);
    ...
    return {
      ...
      cpu: hardware.CPU || '',
      os: hardware.OS || sections.software.find(s=>s.label==='OS Version at Launch')?.value || '',
    };
  }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#0b1222,#0f172a)', color: '#e5e7eb', padding: 16 }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'flex-end' }}>
          <label style={{ color: '#9ca3af', fontSize: 12 }}>AI Provider:</label>
          <select value={provider} onChange={(e)=>setProvider(e.target.value)} style={{ padding: 8, borderRadius: 8, border: '1px solid #334155', background: '#0f172a', color: '#e5e7eb' }}>
            <option value="openrouter">OpenRouter (Gemini)</option>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            {resultsA.length>0 && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px,1fr))', gap: 8 }}>
                {resultsA.map((it)=> (
                  <button key={it.slug} onClick={()=>pick('A', it)} style={{ textAlign: 'left', border: '1px solid #334155', borderRadius: 12, padding: 10, background: '#0f172a', color: '#e5e7eb', cursor: 'pointer' }}>
                    <div style={{ fontWeight: 700, fontSize: 13 }}>{it.phone_name}</div>
                    <div style={{ color: '#9ca3af', fontSize: 12 }}>{it.brand}</div>
                  </button>
                ))}
              </div>
            )}
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            {resultsB.length>0 && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px,1fr))', gap: 8 }}>
                {resultsB.map((it)=> (
                  <button key={it.slug} onClick={()=>pick('B', it)} style={{ textAlign: 'left', border: '1px solid #334155', borderRadius: 12, padding: 10, background: '#0f172a', color: '#e5e7eb', cursor: 'pointer' }}>
                    <div style={{ fontWeight: 700, fontSize: 13 }}>{it.phone_name}</div>
                    <div style={{ color: '#9ca3af', fontSize: 12 }}>{it.brand}</div>
                  </button>
                ))}
